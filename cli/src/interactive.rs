...
"
            ...
            match datalake_type {
                DatalakeType::BlockSampled => {
                    ...
                    let block_range_end: u64 = inquire::Text::new("Block range end")
                        .with_help_message("What is the block range end? (Enter to set default)")
                    ...
                    let collection_opts: Vec<&str> = variants.iter().map(AsRef::as_ref).collect();
                    let collection_ans: &str =
                        Select::new("Sample Property: Select block sample type", collection_opts)
                            .with_help_message("What type of block sample do you want to process?")
                            .prompt()?;
                    let collection_type = BlockSampledCollectionType::from_str(collection_ans)?;
                    ...
                    let sampled_property = match collection_type {
                        ...
                        BlockSampledCollectionType::Storage => {
                            let address = inquire::Text::new("Enter target address")
                                .with_help_message("Enter target address")
                                .prompt()?;
                            let storage_key = inquire::Text::new("Enter target storage key")
                ...
                DatalakeType::TransactionsInBlock => {
                    ...
                    let collection_ans: &str =
                        Select::new("Sample Property: Select block sample type", collection_opts)
                            .with_help_message("What type of block sample do you want to process?")
                            .prompt()?;
                    let collection_type = TransactionsCollectionType::from_str(collection_ans)?;
                    let sampled_property = match collection_type {
    ...
    let aggregate_fn_ctx = match aggregate_fn_id {
        ...
        "SLR" => {
            let target_index: String = inquire::Text::new("Enter the target index to compute SLR")
