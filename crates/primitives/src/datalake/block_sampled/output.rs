...
use crate::datalake::output::{FieldElementVectorUnit, MPTProof, MPTProofFormatted};
...
impl Account {
    pub fn to_cairo_format(&self) -> AccountFormatted {
        ...
        let proofs = self
            ...
            .iter()
            .map(|proof| {
                let proof_chunk_result: Vec<FieldElementVectorUnit> = proof
                    .proof
                    .iter()
                    .map(|proof| FieldElementVectorUnit::from_hex_str(proof).unwrap())
                    .collect();

                let proof_bytes_len = proof_chunk_result.iter().map(|x| x.bytes_len).collect();
                let proof_result: Vec<Vec<FieldElement>> =
                    proof_chunk_result.iter().map(|x| x.felts.clone()).collect();
    ...
    pub fn to_cairo_format(&self) -> StorageFormatted {
        ...
        let proofs = self
            ...
            .map(|proof| {
                ...
                let storage_proof_chunk_result: Vec<FieldElementVectorUnit> = proof
                    .proof
                    .iter()
                    .map(|proof| FieldElementVectorUnit::from_hex_str(proof).unwrap())
                    .collect();
                let storage_proof_bytes_len = storage_proof_chunk_result
                    ...
                    .map(|x| x.bytes_len)
                    .collect();
                let storage_proof_result: Vec<Vec<FieldElement>> = storage_proof_chunk_result
                    .iter()
                    .map(|x| x.felts.clone())
                    .collect();
...
mod tests {
    ...
    fn cairo_format_header() {
        ...
        let formatted_header: HeaderFormatted = original_header.to_cairo_format();
        // let header_string = serde_json::to_string(&formatted_header).unwrap();
        // println!("{:#?}", header_string);
        // // let deserialized_vec: Vec<String> = serde_json::from_str(&header_string).unwrap();
    ...
    //     let header_string = serde_json::to_string(&formatted_account.address).unwrap();
    //     let deserialized_vec: Vec<String> = serde_json::from_str(&header_string).unwrap();
    //     assert_eq!(
    //         deserialized_vec,
    //         vec!["0xaad30603936f2c7f", "0x12f5986a6c3a6b73", "0xd43640f7"]
    //     );
    //     let header_string = serde_json::to_string(&formatted_account.proofs[0].proof).unwrap();
    //     let deserialized_vec: Vec<Vec<String>> = serde_json::from_str(&header_string).unwrap();
