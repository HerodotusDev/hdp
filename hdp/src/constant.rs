pub const DRY_CAIRO_RUN_OUTPUT_FILE: &str = "dry_run_output.json";
pub const SOUND_CAIRO_RUN_OUTPUT_FILE: &str = "cairo_run_output.json";
pub const DEFAULT_DRY_CAIRO_RUN_CAIRO_FILE: &str = "build/contract_dry_run.json";
pub const DEFAULT_SOUND_CAIRO_RUN_CAIRO_FILE: &str = "build/hdp.json";

// Internal Herdootus services URLs
pub const HERODOTUS_RS_INDEXER_URL: &str =
    "https://rs-indexer.api.herodotus.cloud/accumulators/proofs";
pub const HERODOTUS_RS_INDEXER_STAGING_URL: &str =
    "https://staging.rs-indexer.api.herodotus.cloud/accumulators/proofs";
pub const HERODOTUS_PROGRAM_REGISTRY_URL: &str =
    "http://program-registery.api.herodotus.cloud/get-program?program_hash";
